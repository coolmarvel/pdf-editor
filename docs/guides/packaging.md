...
- `release/`는 `.gitignore` 대상이다. 설치 파일은 용량이 커서 git 에 직접 커밋하지 않는다.

## release/ 보관·배포 정책

`release/`는 로컬 빌드 산출물 보관 폴더다. git 에 올리지 않고, 공개 배포가 필요하면 GitHub Release
자산으로 업로드한다.

배포 대상 설치 파일은 최신 버전 기준 3개다.

- Windows: `release/PDF편집기-Setup-<version>.exe`
- macOS Apple Silicon: `release/PDF편집기-<version>-arm64.dmg`
- macOS Intel: `release/PDF편집기-<version>-x64.dmg`

`x64`는 `x86_64`/`amd64`와 같은 Intel Mac 대상이다. 따라서 `amd64.dmg`라는 별도 파일을 만들지 않고,
Electron Builder 표준 명칭인 `x64` 파일명으로 통일한다. 새 버전을 굽고 검증한 뒤에는 이전 버전의 exe/dmg는
혼동을 막기 위해 로컬에서 정리한다. 빌드 중간 산출물(`mac/`, `mac-arm64/`, `*.yml`, `*.blockmap`)과
예전 버전 설치 파일도 git 에 올리지 않는다.

일반 git push 에는 큰 바이너리 파일 제한과 저장소 비대화 문제가 있으므로 설치 파일 커밋은 금지한다.
필요하면 GitHub Release 자산 업로드 스크립트를 사용한다. Git LFS 는 별도 용량/트래픽 관리가 필요하므로
현재 기본 정책으로 쓰지 않는다.
...
- 명령:
  ...
  - `npm run dist:mac:arm64` — Apple Silicon(M1/M2/M3/M4)용
  - `npm run dist:mac:x64` — Intel Mac용 (`x64` = `x86_64` = `amd64`)
- 산출물:
  - `release/PDF편집기-<version>-arm64.dmg`
  - `release/PDF편집기-<version>-x64.dmg`
...
`scripts/dist-mac.cjs`는 `--x64`, `--amd64`, `--intel`, `--arm64`, `MAC_ARCH=x64|arm64`를 지원한다.
Intel Mac 대상은 Electron Builder 기준 명칭인 `x64`로 파일명을 남긴다.
...
- macOS 는 arm64/x64 각각 한 번씩 올린다. dmg 경로를 넘기면 파일명에서 아키텍처를 자동 판별해
  GitHub 자산명을 `PDF-Editor-<v>-arm64.dmg` / `PDF-Editor-<v>-x64.dmg` 로 맞춘다.
- 자산 이름은 ASCII (`PDF-Editor-Setup-<v>.exe`, `PDF-Editor-<v>-arm64.dmg`,
  `PDF-Editor-<v>-x64.dmg`) — 포트폴리오 링크와 1:1.
...
```bash
bash scripts/upload-release-mac.sh "$GH_TOKEN" 1.5.2 release/PDF편집기-1.5.2-arm64.dmg
bash scripts/upload-release-mac.sh "$GH_TOKEN" 1.5.2 release/PDF편집기-1.5.2-x64.dmg
```
